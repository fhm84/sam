...
public class S3FileSystemWrapper implements FileSystemWrapper {
  ...
  public OutputStream openForWrite(String relativePath) throws IOException {
    ...
    return new OutputStream() {
      ...
      private final OutputStream fileOut = Files.newOutputStream(tempFile);
      ...
      public void close() throws IOException {
        ...
        try {
          s3.putObject(
              PutObjectRequest.builder().bucket(bucket).key(key(relativePath)).build(),
  ...
  public InputStream openForRead(String relativePath) throws IOException {
    try {
      return s3.getObject(GetObjectRequest.builder().bucket(bucket).key(key(relativePath)).build());
    } catch (S3Exception e) {
  ...
  public boolean exists(String relativePath) {
    try {
      s3.headObject(HeadObjectRequest.builder().bucket(bucket).key(key(relativePath)).build());
      return true;
    } catch (S3Exception e) {
      return false;
    }
  ...
  public void delete(String relativePath) throws IOException {
    try {
      s3.deleteObject(DeleteObjectRequest.builder().bucket(bucket).key(key(relativePath)).build());
    } catch (S3Exception e) {
