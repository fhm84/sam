 ...
 *
 * <p>This class provides methods to convert a PDF document to an image
 * and crop images. The main method, {@code pdfToImage}, converts the first
 * page of a PDF document to a PNG image and writes it to an output stream.
 * It also includes a commented-out section for converting all pages of a
 * PDF document to images.</p>
 *
 * <p>The {@code cropImage} methods are used to crop a specified percentage
 * of the top and bottom parts of an image and combine them into a new image.</p>
 */
public class DocumentUtils {

    final static int dpi = 90; // DPI for rendering the PDF
    ...
    public static void pdfToImage(InputStream pdfFile, OutputStream outputStream) throws IOException {
        ...
        try (PDDocument document = Loader.loadPDF(pdfFile.readAllBytes())) {
            ...
            // in general, we only need the first page to get the sheets metadata like title, publisher, arranger, composer, ...
