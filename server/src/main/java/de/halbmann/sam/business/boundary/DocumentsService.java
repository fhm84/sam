...
public class DocumentsService {
  ...
  @Inject FileSystemWrapper filesystem;

  @Inject VirusScanner virusScanner;

  @Inject Instance<UploadPolicy> policies;

  @Inject DocumentRepository documentRepository;

  @Inject SheetRepository sheetRepository;

  @Inject InstrumentationRepository instrumentationRepository;
  ...
  public Attachment save(String filename, InputStream inputStream, AttachmentType attachmentType)
      throws IOException, NoSuchAlgorithmException {
    AttachmentEntity uploaded =
        upload(
            filename,
            inputStream,
            Optional.ofNullable(attachmentType).orElse(AttachmentType.UNSPECIFIED));
  ...
  public DocumentDownload loadAttachmentByInstrumentation(String instrumentationId, String docId)
    ...
    InstrumentationEntity instrumentation =
        instrumentationRepository.findById(UUID.fromString(instrumentationId));
    if (instrumentation != null) {
      Optional<UUID> attachmentId =
          instrumentation.getAttachments().stream()
  ...
  public DocumentDownload loadAttachmentBySheet(String sheetId, String docId) throws IOException {
    ...
    if (sheetMusic != null) {
      Optional<UUID> attachmentId =
          sheetMusic.getAttachments().stream()
              .map(AbstractEntity::getId)
              .filter(id -> id.toString().equals(docId))
              .findFirst();
  ...
  void deleteIfUnlinked(UUID documentId) {
    DocumentEntity doc =
        documentRepository.findByIdOptional(documentId).orElseThrow(NotFoundException::new);
