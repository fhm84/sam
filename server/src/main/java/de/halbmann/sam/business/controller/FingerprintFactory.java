...
public class FingerprintFactory {
    ...
    public static FingerprintEntity forSheet(final SheetMusicEntity sheet) {
        ...
        fingerprint.setHashValue(
                FingerprintService.sheetFingerprint(
                        sheet.getTitle(),
                        sheet.getPublisher(),
                        Optional.ofNullable(sheet.getComposer()).map(MusicianEntity::getName).orElse(null),
                        Optional.ofNullable(sheet.getArranger()).map(MusicianEntity::getName).orElse(null)));
        return fingerprint;
    }
