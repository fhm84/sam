...
/**
 * FileType/Extension validator. Based on the (optional) configuration ({@link EnvConsts#FILETYPES}) the documents filename is verified.
 */
@Priority(20)
@ApplicationScoped
public class FileTypePolicy implements UploadPolicy {
    ...
    public void verify(UploadContext context) throws IOException {
        if (fileTypes.isPresent() && !pattern.matcher(context.filename()).matches()) {
            throw new IllegalArgumentException("Filetype for document '" + context.filename() + "' is not allowed to be uploaded!");
        }
