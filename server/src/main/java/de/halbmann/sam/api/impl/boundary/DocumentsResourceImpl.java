...
public class DocumentsResourceImpl implements DocumentsResource {
  ...
  public Response load(String docIdentifier, String ifNoneMatch) {
    ...
    if (instrumentationId != null) {
      try {
        attachment =
            documentsService.loadAttachmentByInstrumentation(instrumentationId, docIdentifier);
      } catch (IOException e) {
    ...
    try {
      ...
      final String encodedFilename =
          URLEncoder.encode(attachment.filename(), StandardCharsets.UTF_8);
      ...
      log.atInfo()
          .setCause(ex)
          .log(() -> String.format("Loading document (%s) resulted in", docIdentifier));
  ...
  public String uploadDocument(FileUploadRequest request) {
    log.atLevel(Level.INFO).log(() -> "File-Upload ... filename: " + request.getFile().fileName());
    ...
    try (InputStream inputStream = Files.newInputStream(request.getFile().uploadedFile())) {
      Attachment attachment =
          documentsService.save(request.getFile().fileName(), inputStream, request.getType());
      return """
                    "filename": "%s",
                    "id": "%s"
                    """
      ...
      log.atWarn().setCause(e).log(() -> "Failed to upload file " + request.getFile().fileName());
      return """
